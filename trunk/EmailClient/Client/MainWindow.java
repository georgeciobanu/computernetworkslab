...
public class MainWindow extends javax.swing.JDialog {
	...
	private void refreshData() {
		...
		EmailTable.setModel(new DefaultTableModel(parseEmailList(emails,
				"Inbox"), columns));
		...
		TableModel model = new DefaultTableModel(
				parseEmailList(emails, "Inbox"), columns) {
	...
	private void createNode(DefaultMutableTreeNode top, String[] path, int pos,
			Folder folder) {
		int i = 0;
		if (((Folder) (top.getUserObject())).toString().equalsIgnoreCase(
			...
			for (i = 0; i < top.getChildCount(); i++) {
				if (((Folder) ((DefaultMutableTreeNode) top.getChildAt(i))
						.getUserObject()).toString().equalsIgnoreCase(
						path[pos + 1]))
					break;
			}
			if (i < top.getChildCount())
				createNode((DefaultMutableTreeNode) top.getChildAt(i), path,
						pos + 1, folder);
		}
	}

	private DefaultMutableTreeNode GenerateTree(Folder[] folders,
		...
		try {
			...
			}
			{
				...
				jPanel1.setLayout(jPanel1Layout);
				{
					...
					// jPanel1.add(FolderTree, BorderLayout.CENTER);
					// FolderTree.setPreferredSize(new java.awt.Dimension(186,
					// 316));
				}
			}
			{
				...
				jPanel2.setLayout(jPanel2Layout);
				{
					...
					EmailTable
							.setPreferredSize(new java.awt.Dimension(418, 316));
				}
			}
			{
				jPanel3 = new JPanel();
				...
				}
				{
					...
					DeleteFolderButton.addMouseListener(new MouseAdapter() {
						public void mouseClicked(MouseEvent evt) {
							DeleteFolderButtonMouseClicked(evt);
						}
					});
				}
				{
					...
					MoveMessageButton.addMouseListener(new MouseAdapter() {
						public void mouseClicked(MouseEvent evt) {
							MoveMessageButtonMouseClicked(evt);
						}
					});
				}
			}
	...
	private void EmailTableMouseClicked(MouseEvent evt) {
		if (evt.getClickCount() > 1) {// double click
			...
			if (container.getMsgType() == MessageTypes.MESSAGE) {
				ViewMessageDialog messageBody = new ViewMessageDialog(null,
						EmailTable.getValueAt(EmailTable.getSelectedRow(), 1)
								.toString(), (Email) container.getPayload());
	...
	private void FolderTreeValueChanged(TreeSelectionEvent evt) {
		...
		if ((TreePath) evt.getNewLeadSelectionPath() == null)
			return;
		DefaultMutableTreeNode currentNode = ((DefaultMutableTreeNode) evt
	...
	private void NewFolderButtonMouseClicked(MouseEvent evt) {
		...
		if (FolderTree.getSelectionCount() != 1) {
			JOptionPane.showMessageDialog(null,
					"Please select only one parent folder");
		} else {
			String name = JOptionPane
					.showInputDialog("Please enter the folder name");
			String path = ((Folder) ((DefaultMutableTreeNode) FolderTree
					.getSelectionPath().getLastPathComponent()).getUserObject())
					.getFldName()
					+ "." + name;
			String[] command = new String[] { "CREATE_FOLDER", path };
			ObjectSender.SendObject(command, MessageTypes.CLIENT_COMMAND,
					getToGateway());
			myContainer container = ObjectSender.WaitForObject(getToGateway());
			...
			if (container.getMsgType() == MessageTypes.CONFIRMATION_OK) {
				...
				DefaultMutableTreeNode top = ((DefaultMutableTreeNode) FolderTree
						.getModel().getRoot());
	...
	private void DeleteFolderButtonMouseClicked(MouseEvent evt) {
		System.out.println("NewFolderButton.mouseClicked, event=" + evt);
		if (FolderTree.getSelectionCount() != 1) {
			JOptionPane.showMessageDialog(null,
					"Please select only one parent folder");
			...
			String path = ((Folder) ((DefaultMutableTreeNode) FolderTree
					.getSelectionPath().getLastPathComponent()).getUserObject())
					.getFldName();
			String[] command = new String[] { "DELETE_FOLDER", path };
			ObjectSender.SendObject(command, MessageTypes.CLIENT_COMMAND,
					getToGateway());
			myContainer container = ObjectSender.WaitForObject(getToGateway());
	...
	private void MoveMessageButtonMouseClicked(MouseEvent evt) {
		System.out.println("MoveMessageButton.mouseClicked, event=" + evt);
